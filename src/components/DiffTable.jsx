...
const DiffTable = (props) => {
	const { rows = [], type = "current", page, rowsPerPage, tableDisabled = false } = props;
	...
	const renderCell = ({ row }) => (
		...
		>
			...
			&nbsp;
			&nbsp;
				<Typography fontWeight="bold">{`${row.finding} (${pluralize("time", sum(Object.values(row.fileInfo).flatMap((file) => file.map((line) => line.num))), true)})`}</Typography>
			</AccordionSummary>
			<AccordionDetails>
				<TableContainer>
					<MuiTable size="small">
							...
							{Object.entries(row.fileInfo).map(([file, lines]) => {
								...
								return(
									<TableRow key={`${row[0]}_${file}`}>
										...
										<TableCell>
											<Typography align="center">{lines.filter((line) => line.num).reduce((sum, line) => sum + line.num, 0)}</Typography>
										</TableCell>
										<TableCell>
											<Typography align="right">
												{lines.map((lineObj, idx) => {
													...
													let displayLine;
													let updatedQueryParams = { ...currentQueryParams, fileName: file };
